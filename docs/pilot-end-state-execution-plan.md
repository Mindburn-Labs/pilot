...
- The Gate 10 eval registry now defines the required production autonomy eval scenarios, durable `eval_runs`/`eval_results`/`eval_evidence_links` records store eval packs, failed evals create blocker tasks, and only passed `real_external_eval` packs can create promotion-eligibility rows without mutating the capability registry. `/api/evals/execute` runs a narrow control-plane proof check for a registered scenario and cannot create promotion eligibility, because full external-world eval orchestration remains incomplete.
...
7. Computer/sandbox production slice: add provider-backed safe command/file/dev-server execution and run the Safe Computer/Sandbox Action Eval.
...
- `control_plane_proof_check` evals are useful regression evidence but are not production promotion evidence. `metadata.executionMode = "real_external_eval"` remains mandatory for production-ready claims.
