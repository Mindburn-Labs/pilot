...
const capabilityRecords = validateCapabilityRecords([
  ...
  },
  {
    ...
    evidence: [
      ...
      'Connector grant/revoke requests, grant/revoke results, token-store requests/results, browser-session store requests/results, validation queue, OAuth initiation requests/results, OAuth callback, OAuth refresh requests/results, and session-delete-intent routes append redacted evidence_items rows without token or session payloads; connector grant/revoke, token storage, browser-session storage, OAuth initiation, OAuth refresh, and session deletion now fail closed before side effects when request evidence persistence fails',
