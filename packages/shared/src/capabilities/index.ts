...
const capabilityRecords = validateCapabilityRecords([
  ...
  },
  {
    ...
    state: 'prototype',
    summary:
      'A canonical evidence_items schema exists and core HELM receipt, agent-loop receipt, subagent spawn, conductor dispatch, A2A inbound dispatch/cancel, Tool Broker, browser session control, browser observation, computer action, managed Telegram send, launch deployment execution, retained tenant hard-delete receipt, retained user-erasure receipt, connector lifecycle, workspace secret mutation, operator mutation, workspace control-plane mutation, approval resolution, compliance control, workspace knowledge memory mutation, workspace-scoped pipeline/ingestion jobs, artifact creation, startup lifecycle, and eval writers append durable proof. Tool Broker now fails closed before elevated tool execution without HELM policy metadata and fails elevated completions closed if evidence persistence fails, but evidence coverage is still not complete for every meaningful action.',
    owner: 'Foundation Agent',
    blockers: [
