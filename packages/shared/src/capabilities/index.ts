...
const capabilityRecords = validateCapabilityRecords([
  {
    ...
    evidence: [
      ...
      'Gateway real_external_eval runner for Domain-to-Deployment verifies build/test computer actions, redacted DNS/hosting target evidence, artifact provenance, live deployment, health check, browser screenshot/DOM proof, rollback-plan evidence, and audit receipts before creating promotion eligibility',
    ],
  ...
  },
  {
    ...
    evidence: [
      ...
      'Managed Telegram claim, webhook, send, token rotation, disable, provisioning-request, and settings paths require HELM governance/evidence appropriate to their effect level and pass explicit effect levels into elevated evaluation',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Gateway real_external_eval runner for Domain-to-Deployment verifies audited build/test computer actions alongside deployment, browser, rollback-plan, and DNS/hosting evidence before creating promotion eligibility',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Agent-loop approval requests persist redacted workspace_approval_requested evidence_items linked to WORKSPACE_APPROVAL_REQUESTED audit_log rows before approval notifications are dispatched',
      'Managed Telegram provisioning requests, claim, webhook configuration, settings updates, token rotation, disable, lead capture, support-message intake, and send-approval requests persist redacted managed_telegram_* evidence_items linked to TELEGRAM_* audit_log rows before state mutation, external Telegram side effects, webhook acknowledgement, approval notification, or reply workflow side effects',
      ...
      'Launch deployment, status update, health-check, and rollback routes persist redacted gateway_launch evidence_items linked to their DEPLOY, DEPLOY_STATUS_UPDATE, DEPLOY_HEALTH_CHECK, and DEPLOY_ROLLBACK audit_log rows before provider calls or deployment state mutations',
      ...
      'Workspace-scoped YC, Startup School, private YC, knowledge ingestion, and opportunity-cluster pipeline workers append redacted pipeline_worker evidence_items rows before script execution and for success/failure',
      'The generic pipeline executor now fails closed before script execution without workspaceId or without durable pipeline_job_started evidence, so private YC, knowledge ingestion, and opportunity-cluster workers cannot silently run without durable workspace-scoped evidence',
      ...
      'Connector grant/revoke requests, grant/revoke results, token-store requests/results, browser-session store requests/results, validation queue, OAuth initiation requests/results, OAuth callback requests/results, OAuth refresh requests/results, and session-delete-intent routes append redacted evidence_items rows without token or session payloads; connector grant/revoke, token storage, browser-session storage, OAuth initiation, OAuth callback, OAuth refresh, and session deletion now fail closed before side effects when request evidence persistence fails',
  ...
  },
  {
    ...
    evidence: [
      ...
      'Gateway real_external_eval runner for Company Formation Prep verifies completed lifecycle node evidence, brokered formation comparison and draft packet, read-only formation-provider browser extraction, explicit signature/filing/identity/payment/legal human gates, no legal filing/payment/identity submission, and legal/financial audit receipts before creating promotion eligibility',
    ],
    evalRequirement:
      'Full Startup Launch Eval, Stripe Setup Prep Eval, and Company Formation Prep Eval',
    updatedAt: CAPABILITY_REGISTRY_UPDATED_AT,
  },
  {
