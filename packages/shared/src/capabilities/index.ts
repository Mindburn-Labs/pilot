...
const capabilityRecords = validateCapabilityRecords([
  ...
  },
  {
    ...
    evidence: [
      ...
      'Connector grant/revoke requests, grant/revoke results, token metadata, browser-session metadata, validation queue, OAuth initiation, callback, refresh, and session-delete-intent routes append redacted evidence_items rows without token or session payloads; connector grant/revoke and session deletion now fail closed before side effects when request evidence persistence fails',
